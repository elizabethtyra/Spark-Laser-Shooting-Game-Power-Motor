...
MD_MAX72XX livesMatrix = MD_MAX72XX(HARDWARE_TYPE, DATA_PIN, CLK_PIN, CS_PIN, MAX_DEVICES);
...
void updateLives(int lives) {
  if (lives == 2) {
    for (int i = 0; i < 8; i++) {
      livesMatrix.setRow(2, 2, i, heart_empty[i]);
    }
  }
  if (lives == 1) {
    for (int i = 0; i < 8; i++) {
      livesMatrix.setRow(1, 1, i, heart_empty[i]);
    }
  }
  if (lives == 0) {
    for (int i = 0; i < 8; i++) {
      livesMatrix.setRow(0, 0, i, heart_empty[i]);
    }
  }
}
